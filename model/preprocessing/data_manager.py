...
def load_dataset(*, client_file_name: str, price_file_name: str) -> pd.DataFrame:
    ...
    dataframe['price_change_energy'] = dataframe['offpeak_diff_dec_january_energy'].apply(price_change)
    dataframe['price_change_power'] = dataframe['offpeak_diff_dec_january_power'].apply(price_change)

    # Dropping the first feature.
    dataframe = dataframe.drop(['offpeak_diff_dec_january_energy', 'offpeak_diff_dec_january_power'], axis=1)
...
def price_data_trans(df:pd.DataFrame):
    ...
    monthly_price_by_id = df.groupby(['id', 'price_date']).agg(
